...
#include "vibikey_windows_hook.h"
#include <stdio.h>

/* Global state */
vibikey_input_mode_t vibikey_current_mode = VIBIKEY_INPUT_BACKSPACE;
int vibikey_hotkey_backspace = VIBIKEY_HOTKEY_BACKSPACE;
int vibikey_hotkey_inline = VIBIKEY_HOTKEY_INLINE;
int vibikey_hotkey_reset = VIBIKEY_HOTKEY_RESET;

HHOOK g_hKeyboardHook = NULL;
HHOOK g_hMouseHook = NULL;

char g_input_buffer[VIBIKEY_INPUT_BUFFER_SIZE];
...
int vibikey_windows_init(void) {
    g_hKeyboardHook = SetWindowsHookEx(
        ...
        vibikey_kbd_hook_proc,
        GetModuleHandle(NULL),
        0
    );

    if (g_hKeyboardHook == NULL) {
        vibikey_log(VIBIKEY_LOG_ERROR, "Failed to install keyboard hook (error %lu)", GetLastError());
        return -1;
    }

    vibikey_log(VIBIKEY_LOG_INFO, "Windows keyboard hook installed successfully");
    return 0;
}
...
int vibikey_windows_cleanup(void) {
    ...
    vibikey_log(VIBIKEY_LOG_INFO, "Windows hooks removed");
    return 0;
}
...
void vibikey_windows_set_mode(vibikey_input_mode_t mode) {
    vibikey_current_mode = mode;
    vibikey_log(VIBIKEY_LOG_INFO, "Input mode set to %d", mode);

    /* Reset buffer when switching modes */
    vibikey_reset_input_buffer();
}
...
LRESULT CALLBACK vibikey_kbd_hook_proc(int nCode, WPARAM wParam, LPARAM lParam) {
    if (nCode >= 0) {
        ...
        if (wParam == WM_KEYDOWN || wParam == WM_SYSKEYDOWN) {
            ...
            /* Check for hotkeys */
            if (vkCode == vibikey_hotkey_backspace) {
                vibikey_log(VIBIKEY_LOG_DEBUG, "Backspace hotkey pressed");
                vibikey_windows_set_mode(VIBIKEY_INPUT_BACKSPACE);
                return CallNextHookEx(g_hKeyboardHook, nCode, wParam, lParam);
            }

            if (vkCode == vibikey_hotkey_inline) {
                vibikey_log(VIBIKEY_LOG_DEBUG, "Inline hotkey pressed");
                vibikey_windows_set_mode(VIBIKEY_INPUT_INLINE);
                return CallNextHookEx(g_hKeyboardHook, nCode, wParam, lParam);
            }

            if (vkCode == vibikey_hotkey_reset) {
                vibikey_log(VIBIKEY_LOG_DEBUG, "Reset hotkey pressed");
                vibikey_reset_input_buffer();
                return CallNextHookEx(g_hKeyboardHook, nCode, wParam, lParam);
            }
            ...
            if (vibikey_current_mode == VIBIKEY_INPUT_INLINE) {
 ...
 */
static void process_character_input(char ch) {
    if (g_input_length >= VIBIKEY_INPUT_BUFFER_SIZE - 1) {
        vibikey_reset_input_buffer();
    }
    ...
    vibikey_log(VIBIKEY_LOG_DEBUG, "Char '%c' added to buffer: %s", ch, g_input_buffer);
}
...
static void process_inline_buffer(void) {
    ...
    int out_len = vibikey_convert_word(g_input_buffer, output, VIBIKEY_OUTPUT_BUFFER_SIZE, method);

    if (out_len > 0) {
        vibikey_log(VIBIKEY_LOG_DEBUG, "Inline conversion: %s → %s", g_input_buffer, output);
...
void vibikey_process_input_buffer(void) {
    ...
    if (out_len > 0) {
        vibikey_log(VIBIKEY_LOG_INFO, "Backspace method commit: %s → %s", g_input_buffer, output);
